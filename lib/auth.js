...
import { toRuntimeApiUrl } from "./api-base";
...
async function createAdminSession(idToken) {
  try {
    const response = await fetch(toRuntimeApiUrl("/api/admin/session"), {
      method: "POST",
...
async function clearAdminSession() {
  try {
    await fetch(toRuntimeApiUrl("/api/admin/session"), { method: "DELETE" });
  } catch {
