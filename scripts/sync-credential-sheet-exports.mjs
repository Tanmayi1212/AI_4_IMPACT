...
import {
  ...
  CREDENTIAL_EXPORT_EVENTS_COLLECTION,
} from "../src/app/api/admin/_utils/credential-sheet-export.js";
...
async function main() {
  ...
  if (snapshot.empty) {
    console.log("No credential export events to sync.");
    return;
  }
  ...
  for (const doc of snapshot.docs) {
    ...
    const result = await attemptCredentialSheetExportSync({
      eventId: doc.id,
      force,
    });
...
main().catch((error) => {
  console.error(`Credential export sync failed: ${error.message}`);
  process.exit(1);
});
